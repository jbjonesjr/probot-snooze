...
module.exports = class Freeze {
  ...
  freezable(commentBody) {
    const probotExpression = '@probot(-freeze[bot])?';
    const freezeExpression = '(freeze|archive|suspend)';
    if (commentBody.body.match(probotExpression)) {
      if (commentBody.body.match(freezeExpression)) {
        // TODO: Bug when freeze only exists in a call to probot-freeze
        return true;
      }
    }
    return false;
